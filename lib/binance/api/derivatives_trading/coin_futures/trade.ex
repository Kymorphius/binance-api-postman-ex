defmodule Binance.API.DerivativesTrading.CoinFutures.Trade do
  ...
  Requires signature: true
  """
  def all_orders_v1(client, symbol: symbol, pair: pair, orderId: orderId, startTime: startTime, endTime: endTime, limit: limit, recvWindow: recvWindow) do
    with {:ok, base_url} <- base_url(client.env) do
      {:ok, request} =
        Binance.RequestBuilder.build(%{
          client: client,
  ...
  Requires signature: true
  """
  def current_all_open_orders_v1(client, symbol: symbol, pair: pair, recvWindow: recvWindow) do
    with {:ok, base_url} <- base_url(client.env) do
      {:ok, request} =
        Binance.RequestBuilder.build(%{
          client: client,
  ...
  Requires signature: true
  """
  def modify_order_v1(client, symbol, side, orderId: orderId, origClientOrderId: origClientOrderId, quantity: quantity, price: price, priceMatch: priceMatch, recvWindow: recvWindow) do
    with {:ok, base_url} <- base_url(client.env) do
      {:ok, request} =
        Binance.RequestBuilder.build(%{
          client: client,
  ...
  Requires signature: true
  """
  def position_information_v1(client, marginAsset: marginAsset, pair: pair, recvWindow: recvWindow) do
    with {:ok, base_url} <- base_url(client.env) do
      {:ok, request} =
        Binance.RequestBuilder.build(%{
          client: client,
  ...
  Requires signature: true
  """
  def account_trade_list_v1(client, symbol: symbol, pair: pair, orderId: orderId, startTime: startTime, endTime: endTime, fromId: fromId, limit: limit, recvWindow: recvWindow) do
    with {:ok, base_url} <- base_url(client.env) do
      {:ok, request} =
        Binance.RequestBuilder.build(%{
          client: client,
          ...
          query: [symbol: symbol, pair: pair, orderId: orderId, startTime: startTime, endTime: endTime, fromId: fromId, limit: limit, recvWindow: recvWindow, timestamp: nil],
          headers: [{"X-MBX-APIKEY", ""}, {"Accept", "application/json"}],
          body: %{mode: "urlencoded", urlencoded: []}
        })
